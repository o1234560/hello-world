      ...
      <div>
        ...
        <div class="film-showDate" v-if="info">
          ...
          <div class="film-showDate-content">
            <li
              class="film-showDate-content-item"
...
export default {
  ...
  created () {
    http({
      url: `/gateway/?cinemaId=${this.$route.params.id}&k=${Math.floor(Math.random() * 10000000)}`,
      headers: {
  ...
  methods: {
    ...
    getSchedules (date) {
      // console.log(date)
      http({
        url: `/gateway/?filmId=${this.info.filmId}&cinemaId=${this.$route.params.id}&date=${date}&k=${Math.floor(Math.random() * 10000000)}`,
